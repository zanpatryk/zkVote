...
contract ZKVoteBenchmark is BenchmarkBase {
    ...
    function createPoll() internal override returns (uint256 pollId) {
        ...
        options[0] = "A";
        options[1] = "B";
        bytes memory voteStorageConfig =
            abi.encode([uint256(123), 456], address(localSys.voteV), address(localSys.tallyV));
        bytes memory createData = abi.encodeCall(
            BenchmarkActor.createPoll,
            ("ZK Benchmark", "Desc", options, voteStorageConfig, "", address(0), sys.voteStorageAddr)
        );
