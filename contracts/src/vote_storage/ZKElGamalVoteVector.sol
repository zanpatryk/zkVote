...
contract ZKElGamalVoteVector is IVoteStorage, IZKElGamalVoteVector, Ownable {
    ...
    function initPoll(uint256 pollId, bytes calldata initData) external override {
        (uint256[2] memory publicKey, address voteVerifier, address tallyVerifier) =
            abi.decode(initData, (uint256[2], address, address));
        _initPoll(pollId, publicKey, voteVerifier, tallyVerifier);
    }

    function _initPoll(uint256 pollId, uint256[2] memory publicKey, address voteVerifier, address tallyVerifier)
        internal
    {
        ...
        (bool success, bytes memory data) =
            pollManager.staticcall(abi.encodeWithSignature("getPollOwner(uint256)", pollId));
    ...
    function castVote(uint256 pollId, address voter, bytes calldata voteData) external override returns (uint256) {
        if (msg.sender != pollManager && msg.sender != owner()) revert ZKElGamalVoteVector__NotPollOwner();

        (uint256[64] memory encVote, ZKVoteLib.Proof memory proof) =
            abi.decode(voteData, (uint256[64], ZKVoteLib.Proof));
    ...
    function _castEncryptedVote(uint256 pollId, address voter, uint256[64] memory encVote, ZKVoteLib.Proof memory proof)
        internal
        returns (uint256)
    {
        PollState storage poll = polls[pollId];
        ...
        // Verify vote proof
        if (!IZKElGamalVoteVectorVerifier(poll.voteVerifier).verifyProof(proof.a, proof.b, proof.c, pubSignals)) {
        ...
        for (uint256 i = 0; i < N; i++) {
            uint256 base = i * 4;
            poll.aggC1[i] = ZKVoteLib.addPoint(poll.aggC1[i], [encVote[base], encVote[base + 1]]);
            poll.aggC2[i] = ZKVoteLib.addPoint(poll.aggC2[i], [encVote[base + 2], encVote[base + 3]]);
        }
        ...
        s_votes[voteId] = EncryptedVote({voteId: voteId, pollId: pollId, voter: voter, encVote: encVote});
     ...
     */
    function publishResults(uint256 pollId, uint256[16] calldata tally, ZKVoteLib.Proof calldata tallyProof)
        external
        override
        onlyOwner
    {
        ...
        if (!IZKElGamalTallyVerifier(poll.tallyVerifier)
                .verifyProof(tallyProof.a, tallyProof.b, tallyProof.c, pubSignals)) {
